...
def generate_launch_description():
    ...
    gazebo_pkg_share = launch_ros.substitutions.FindPackageShare(package='bal_robot_gazebo').find('bal_robot_gazebo')
    default_model_path = os.path.join(pkg_share, 'urdf/bal_robot.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'rviz/bal_robot_navigation.rviz')
    world_path=os.path.join(gazebo_pkg_share, 'worlds/room2.sdf'),
    sdf_path=os.path.join(pkg_share, 'urdf/bal_robot', 'model.sdf'),
    ...
    arguments=['-entity', 'bal_robot', '-topic', 'robot_description'],
    parameters= [{'use_sim_time': use_sim_time}],
    output='screen'
    )
